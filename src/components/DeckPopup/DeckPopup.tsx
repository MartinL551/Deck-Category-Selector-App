      ...
      <View className={styles.modalContainer}>
        <View className={styles.deckTypePopup}>
          ...
          <Animated.View style={[animatedTitleStyle]}>
            <Text className={styles.popupTitle}>{t(typeKey ?? 'ORG', { ns: 'histerTitles' })}</Text>
          </Animated.View>
          <Text className={styles.popupBlurb}>{t(typeKey ?? 'ORG', { ns: 'histerBlurbs' })}</Text>
        </View>
      </View>
    </Modal>
  );
...
const styles = {
  ...
  popupTitle:
    'text-center text-3xl w-full my-5 rounded-[10] px-3 py-1 bg-onairred text-white font-bitcount-bold',
  popupBlurb:
    'text-center text-medium my-5 py-2 px-5 w-full bg-white mt-[50px] font-bitcount-bold rounded-[10]',

...
export function TutorialOverlay({ visible, onDismiss }: Props) {
  ...
  return (
    <Pressable onPress={onDismiss} className={styles.overlay}>
      <View className={styles.contentContainer}>
        <Text className={styles.tutorialText}>{t('tutorialText', { ns: 'tutorial' })}</Text>
        ...
        <Text className={styles.tutorialText}>{t('continue', { ns: 'tutorial' })}</Text>
      </View>
    </Pressable>
  );
}
...
const styles = {
  overlay:
    'absolute top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50 z-50 items-center justify-center',
